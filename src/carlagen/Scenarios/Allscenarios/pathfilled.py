...
class PathFilled(BasicScenario):
    """
    Desc: TODO
    Special: 补充TODO的数据
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self.num = 0
        self.random_num = random.uniform(1,2)
        ...
        super().__init__("PathFilled", ego_vehicles, config, world, randomize, debug_mode,
    ...
    def interfuser_tick_autopilot(self):
        ...
        # ipdb.set_trace()
        if math.sqrt((x_coordinate - self.starting_wp.transform.location.x) ** 2 +  (y_coordinate - self.starting_wp.transform.location.y) ** 2) >= self.random_distance \
                and self.num < self.random_num:
            self.obstacle_location ,npc,self.spawn_npc_point_location = spawn_pedestrian(self._world, self.starting_wp)
            self.num += 1
        #control_pedestrian(npc, spawn_npc_point)
        ...
        if self.obstacle_location is not None:
            ...
            delta_x = self.spawn_npc_point_location.x - ego_location.x
            delta_y = self.spawn_npc_point_location.y - ego_location.y
            # 计算向量与车辆朝向方向的夹角（弧度）
            angle_rad = math.atan2(delta_y, delta_x) - math.atan2(ego_direction.y, ego_direction.x)

            # distance = math.sqrt((self.spawn_npc_point_location.x - cur_ego_loc.x )** 2 + (self.spawn_npc_point_location.y - cur_ego_loc.y )** 2)
            ...
            if distance > 10 :
                ...
                self._tf_set_ego_speed(self.init_speed)
            elif 10 >= distance and math.fabs(angle_rad) < math.atan(cur_wp.lane_width/ 20):# 观测到前方有静止的障碍物
                ego_stage = '#dynamic1'
    ...
    def _initialize_actors(self, config):
        ...
        # self.obstacle_location = spawn_pedestrian(self._world, self.starting_wp)
    ...
    def _create_behavior(self):
        root = py_trees.composites.Parallel(name="PathFilled",
                                            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        c1 = py_trees.composites.Sequence(name="PathFilled_c1")
