...
class FollowingStopCondition(AtomicCondition):
    def __init__(self, debug=False):

        self.scenario_name = 'FollowingStop'

        self.min_distance_to_junction = 50  #

        super(FollowingStopCondition, self).__init__(debug)
...
class FollowingStop(BasicScenario):
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        super().__init__("FollowingStop", ego_vehicles, config, world, randomize, debug_mode,
    ...
    def _update_scenario_settings(self):
        ...
        planA_follow_init_speed = random.randint(planA_ego_init_speed, planA_ego_init_speed + 3)
        planA_decrease_start_point_distance = random.randint(50, 100)
        planA_decrease_end_point_distance = random.randint(planA_decrease_start_point_distance + 25, planA_decrease_start_point_distance + 45)
    ...
    def drive_planA(self):
        ...
        if self.ego_drive_distance > 5 and abs(ego_speed) < 0.5:
            ...
            elif self.default_settings['planA']['decrease_start_point']['distance'] <= self.follow_drive_distance <= self.default_settings['planA']['decrease_end_point']['distance']:
                # 线性减速
                p1 = (self.default_settings['planA']['decrease_start_point']['distance'], self.default_settings['planA']['decrease_start_point']['speed'])
                p2 = (self.default_settings['planA']['decrease_end_point']['distance'], self.default_settings['planA']['decrease_end_point']['speed'])
    ...
    def drive_planB(self):
        ...
        if self.ego_drive_distance > 5 and abs(ego_speed) < 0.5:
            ...
            elif self.default_settings['planB']['decrease_start_point']['distance'] <= self.follow_drive_distance <= self.default_settings['planB']['decrease_end_point']['distance']:
                # 线性减速
                p1 = (self.default_settings['planB']['decrease_start_point']['distance'], self.default_settings['planB']['decrease_start_point']['speed'])
                p2 = (self.default_settings['planB']['decrease_end_point']['distance'], self.default_settings['planB']['decrease_end_point']['speed'])
                target_speed = calculate_y_for_given_x(self.follow_drive_distance, p1, p2)
                self._tf_set_actor_speed(target_speed, self.following_car)
            else:
                self._tf_set_actor_speed(0, self.following_car)
    ...
    def _create_behavior(self):
        root = py_trees.composites.Parallel(name="FollowingStop", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        ...
        c1 = py_trees.composites.Sequence(name="FollowingStop_c1")
