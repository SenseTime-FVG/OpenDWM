...
class PathRightPes(BasicScenario):
    """
    Desc: TODO
    Special: 补充TODO的数据
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        super().__init__("PathRightPes", ego_vehicles, config, world, randomize, debug_mode,
                         criteria_enable=criteria_enable, uniad=uniad, interfuser=interfuser,assign_distribution = None)
    ...
    def interfuser_tick_autopilot(self):
        ...
        # ipdb.set_trace()
        control_pedestrian_right(npc, self.spawn_npc_point)
        self.obstacle_location = npc.get_location()
        ...
        # print("self.obstacle_location  = ",self.obstacle_location )
        if self.obstacle_location is not None:
            distance = round( self.obstacle_location.distance(cur_ego_loc), 2)
            distance_r = math.sqrt((self.obstacle_location.x - cur_ego_loc.x) ** 2 + (self.obstacle_location.y - cur_ego_loc.y) ** 2)
            ego_direction = ego_transform.rotation.get_forward_vector()
            delta_x = self.obstacle_location.x - ego_location.x
            delta_y = self.obstacle_location.y - ego_location.y
            # 计算向量与车辆朝向方向的夹角（弧度）
            ...
            # print("---------------------",(math.fabs(math.sin(angle_rad))))
            # print("distance_r-----",distance_r)
            if distance_r >= (cur_wp.lane_width/2 +cur_wp.lane_width / 3.0):   #     行驶过行人
                # print("distance_R",distance_r)
                # print("angle_rad",angle_rad)
                if distance > 15 and self.stage == 0 :
                    self.traffic_manager.ignore_walkers_percentage(ego, 100)
                    ego_stage = '#fix1'
                    ...
                    self.traffic_manager.ignore_walkers_percentage(ego, 100)
                    ego_stage = '#fix2'
                    reason = 'because there is a pedestrian walking along the path, so decelerating.'
                    self._tf_set_ego_speed(12)
                elif distance > 5 and self.stage == 0: # and math.fabs(angle_rad) <= math.asin((cur_wp.lane_width/2 )/ distance):#  观测到前方有行人
                    if self.avoid_sign is False:
                        # print("math.fabs(angle_rad) =   ", math.fabs(angle_rad))
                        # print("math.atan(1.5 / 10) = ", math.atan(1.5 / 10))
                        self.traffic_manager.ignore_walkers_percentage(ego, 100)
                        self.offset_m = cur_wp.lane_width / 4.0
                ...
                self.traffic_manager.ignore_walkers_percentage(ego, 0)
                self._tf_set_ego_speed(self.init_speed)
        else:
            self._tf_set_ego_speed(self.init_speed)
    ...
    def _initialize_actors(self, config):
        ...
        self.obstacle_location,npc,self.spawn_npc_point = spawn_pedestrian_right(self._world, self.starting_wp)
        self.npc_all.append(npc)
        # For: 右侧交通流
    ...
    def _create_behavior(self):
        root = py_trees.composites.Parallel(name="PathRightPes",
                                            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        c1 = py_trees.composites.Sequence(name="PathRightPes_c1")
        # yaogai
