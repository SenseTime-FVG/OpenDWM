...
class RedlightVehicleCondition(AtomicCondition):
    def __init__(self, debug=False):

        self.scenario_name = "RedlightVehilce"

        self.min_distance_to_junction = 50  #

        super(RedlightVehicleCondition, self).__init__(debug)
...
class RedlightVehicle(BasicScenario):
    """
    ...
    def sample_drive_plan(self):
        ...
        if distance_to_junc > 150:
            ...
            self.following_car_wp = move_waypoint_forward(
                self.starting_wp, move_distance
            )
