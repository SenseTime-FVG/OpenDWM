...
from Initialization.Standardimport.scenariomanager.scenarioatomics.atomic_behaviors import calculate_distance
...
from Initialization.Standardimport.srunner_tools.scenario_helper import get_distance_along_route, get_distance_between_actors
...
class ScenarioNPCTriggerFunction(AtomicCondition):
    def __init__(self, scenario_self, name="ScenarioNPCTriggerFunction"):
        """
        Setup trigger distance
        """
        super(ScenarioNPCTriggerFunction, self).__init__(name)
        self.logger.debug("%s.__init__()" % self.__class__.__name__)
        self.scenario_self = scenario_self

    def update(self):
        """
        Check if the function returns True
        """
        self.scenario_self.start_npc()
        new_status = py_trees.common.Status.SUCCESS
...
class InTriggerDistanceToVehicle(AtomicCondition):
    ...
    def update(self):
        ...
        if self._comparison_operator(distance, self._distance):
            # print(f'++++++++ Distance triggered: {distance}  {self._distance}')
            new_status = py_trees.common.Status.SUCCESS
...
class InTriggerDistanceToLocation(AtomicCondition):
    ...
    def update(self):
        ...
        if self._comparison_operator(calculate_distance(
            ...
            print(f'\nscenario triggered')
            print(f'++++++++++++ InTriggerDistanceToLocation triggered')
            new_status = py_trees.common.Status.SUCCESS
...
class InTriggerDistanceToNextIntersection(AtomicCondition):
    ...
    def update(self):
        ...
        if distance < self._distance:
            print(f'InTriggerDistanceToNextIntersection triggered')
            new_status = py_trees.common.Status.SUCCESS
...
class InTimeToArrivalToLocation(AtomicCondition):
    ...
    def update(self):
        ...
        if self._comparison_operator(time_to_arrival, self._time):
            print(f'++++++++ InTimeToArrivalToLocation triggered')
            new_status = py_trees.common.Status.SUCCESS
...
class DriveDistance(AtomicCondition):
    ...
    def update(self):
        ...
        if self._distance > self._target_distance:
            print(f'DriveDistance triggered')
            new_status = py_trees.common.Status.SUCCESS
...
class TimeoutRaiseException(AtomicCondition):
    ...
    def __init__(self, duration):
        ...
        Setup actor
        """
        super(TimeoutRaiseException, self).__init__(name="TimeoutRaiseException")
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._duration = duration
        self._start_time = 0
    ...
    def initialise(self):
        ...
        """
        self._start_time = GameTime.get_time()
        super(TimeoutRaiseException, self).initialise()
    ...
    def update(self):
        ...
        if GameTime.get_time() - self._start_time > self._duration:
            raise Exception("TimeoutRaiseException: Timeout reached")
