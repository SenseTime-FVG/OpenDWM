...
from Initialization.Standardimport.traffic_events import TrafficEvent, TrafficEventType
...
class CollisionTest(Criterion):
    ...
    def _count_collisions(self, event):     # pylint: disable=too-many-return-statements
        ...
        agent = CarlaDataProvider._agent_instance
        if agent is not None:
            collision_event.set_message(
            "Agent collided against object with type={} and id={} at (x={}, y={}, z={}, at step={} with status={})".format(
    ...
    def _set_traffic_event(self):
        ...
        if agent is not None:
            self._traffic_event.set_message(
                ...
                "({}% of the completed route), at step={} with status={}".format(
...
class RouteCompletionTest(Criterion):
    ...
    def _get_acummulated_percentages(self):
        ...
        # return [1 for x in accum_meters]
                ...
                if tail_wp.road_id == wp.road_id and tail_wp.lane_id == wp.lane_id and ve_dir.dot(wp_dir) > 0:
                    ...
                    if self.is_vehicle_crossing_line((tail_close_pt, tail_far_pt), (lft_lane_wp, rgt_lane_wp)):
                        ...
                        if agent is not None:
                            ...
                            "Agent ran a red light {} at (x={}, y={}, z={}, at step={} with status={})".format(
                                traffic_light.id,
                                round(location.x, 3),
                                round(location.y, 3),
                                round(location.z, 3),
            ...
            if not self._stop_completed:
                ...
                if agent is not None:
                    running_stop_event.set_message(
                        "Agent ran a stop with id={} at (x={}, y={}, z={}, at step={} with status={})".format(
