...
import carla
from Initialization.Standardimport.agents.navigation.basic_agent import BasicAgent
from Initialization.Standardimport.agents.navigation.constant_velocity_agent import ConstantVelocityAgent
from Initialization.Standardimport.agents.navigation.local_planner import RoadOption, LocalPlanner
from Initialization.Standardimport.agents.tools.misc import is_within_distance, get_speed

from Initialization.Standardimport.scenariomanager.carla_data_provider import CarlaDataProvider
from Initialization.Standardimport.scenariomanager.actorcontrols.actor_control import ActorControl
from Initialization.Standardimport.scenariomanager.timer import GameTime
from Initialization.Standardimport.srunner_tools.scenario_helper import detect_lane_obstacle
from Initialization.Standardimport.srunner_tools.scenario_helper import generate_target_waypoint_list_multilane


import Initialization.Standardimport.srunner_tools as sr_tools
...
class GhostKeepVelocity(AtomicBehavior):
    ...
    def __init__(self, actor, target_velocity, duration=float("inf"), distance=float("inf"), name="KeepVelocity"):
        ...
        and target velocity
        """
        super(GhostKeepVelocity, self).__init__(name, actor)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._target_velocity = target_velocity
    ...
    def initialise(self):
        ...
        super(GhostKeepVelocity, self).initialise()
    ...
    def terminate(self, new_status):
        ...
        if self._actor is not None and self._actor.is_alive:
            self._actor.apply_control(self._control)
        super(GhostKeepVelocity, self).terminate(new_status)
...
class OppositeActorFlow(AtomicBehavior):
    ...
    def _spawn_actor(self):
        ...
        traffic_manager.set_desired_speed(actor, self._speed)
        traffic_manager.set_path(actor, [self._route[-1][0].transform.location])
        actor.set_autopilot(True)

        # controller = BasicAgent(actor, 3.6 * self._speed, self._opt_dict, self._map, self._grp)
        # controller.set_global_plan(self._route)
        self._actor_list.append([actor, None])
    ...
    def terminate(self, new_status):
        ...
        for actor, _ in self._actor_list:
            try:
                print(f'\nactor flow destroy actor: {actor}\n')
...
class ScenarioTriggerer(AtomicBehavior):
    ...
    def __init__(self, actor, route, blackboard_list, distance, debug=False, name="ScenarioTriggerer", repeat_scenarios=False):
        """
        Setup class members
        """
