...
def pts_transfer(src_pts, height=0):
    n, pt_shape = src_pts.shape
    src_pts = np.transpose(src_pts)
    homogeneous = np.ones((n))[None, :]
    if pt_shape==3:
        res = np.concatenate([src_pts, homogeneous], axis=0)
    else:
        height = np.array([height] * n)[None, :]
        res = np.concatenate([src_pts, height, homogeneous], axis=0)
    return res
...
def project_pvb_to_camera(points, camera_intrinsic, sensor_extrinsic, return_deep=False):
    # sensor to camera, 4xN
    camera_points = sensor_extrinsic.dot(np.array(points))
    ...
    camera_points_copy = np.copy(camera_points)
    for line in BBOX_LINES:
        p1 = camera_points_copy[line[0] - 1].copy()
        p2 = camera_points_copy[line[1] - 1].copy()
        if p2[2] < 0.01 and p1[2] >= 0.01:
            k = (0.01 - p2[2]) / (p1[2] - p2[2])
            p2 += k * (p1 - p2)
            camera_points[line[1] - 1] = p2
        elif p1[2] < 0.01 and p2[2] >= 0.01:
            k = (0.01 - p1[2]) / (p2[2] - p1[2])
            p1 += k * (p2 - p1)
            camera_points[line[0] - 1] = p1
    camera_points = camera_points.T

    # camera to image, 3xN
    img_points = camera_intrinsic.dot(camera_points)
    if (img_points[2, :] >= 0).sum() < 8 and (img_points[2, :] >= 0).sum() > 0:
        import ipdb
        ipdb.set_trace()
    img_points = img_points[..., img_points[2, :] >= 0]  # 只保留深度为正的点
    img_points[0, :] /= img_points[2, :]
...
def line_to_dense(pt1, pt2, step=1.0):
        s = pt2 - pt1
        s_norm = np.linalg.norm(s)
        s /= s_norm

        dense_line = []
        for i in range(int(s_norm / step)):
            pt = pt1 + i * step * s
            dense_line.append(pt)
        dense_line.append(pt2)
        return dense_line

def linestrip_to_dense(points, step=1.0):
    # return lanes
    dense_linestrip = []
    ...
    for idx in range(len(points)-1):
        pt1 = points[idx]
        pt2 = points[idx+1]
        if np.isnan(pt1[0]) or np.isnan(pt2[0]) or (pt1==pt2).all():
            continue
        dense_linestrip += line_to_dense(pt1, pt2, step)

    return np.array(dense_linestrip)
class DashedImageDraw(ImageDraw.ImageDraw):
    ...
    def thick_line(self, xy, direction, fill=None, width=0):
        ...
        if xy[0] != xy[1]:
            ...
            self.line([(x3, y3), (x4, y4)], fill = fill, width = width)
        return   
...
def draw_dashed_pts(img, pts, color=(0, 0, 255), thickness=1, dash=(2,2)):
    if img is None:
        import ipdb
        ipdb.set_trace()
    d = DashedImageDraw(img)
    # Convert points to integer numpy array
    points = np.array([[int(x), int(y)] for x, y in pts]).astype(np.int32)
    # Get the width of the image
    img_width = img.size[0]
    half_width = img_width / 2
    
    # Iterate through the points and draw lines conditionally
    for i in range(len(points) - 1):
        pt1 = points[i]
        pt2 = points[i + 1]
        
        # Check if the distance between the points is greater than half of the image width
        l = np.linalg.norm(pt1 - pt2)
        if l <= half_width and l > 0:
            d.dashed_line([tuple(pt1), tuple(pt2)], dash, color, thickness)
        
def draw_pts(img, pts, color=(0, 0, 255), thickness=1):
    d = DashedImageDraw(img)
    # Convert points to integer numpy array
    points = np.array([[int(x), int(y)] for x, y in pts]).astype(np.int32)
    # Get the width of the image
    ...
    for i in range(len(points) - 1):
        ...
        l = np.linalg.norm(pt1 - pt2)
        if l <= half_width and l > 0:
            d.line([tuple(pt1), tuple(pt2)], fill = color, width = thickness)
